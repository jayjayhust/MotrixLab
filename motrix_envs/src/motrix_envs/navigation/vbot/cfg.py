...
class Asset:
    ...
    ground_name = "ground"
    goal_name = "V_End_Point"  # 目标位置名称，注意要加V_这个前缀（在scene_section001.xml中进行了定义）
...
class VBotSection012EnvCfg(VBotStairsEnvCfg):
    ...
    max_episode_seconds: float = 40.0  # 地形复杂：从20秒增加到40秒
    max_episode_steps: int = 4000  # 地形复杂：从2000步增加到4000步（本回合步数到达后会重新开始训练）
    sim_dt: float = 0.01    # 仿真步长 10ms = 100Hz
    ctrl_dt: float = 0.01
    reset_yaw_scale: float = 0.1
    max_dof_vel: float = 100.0  # 最大关节速度阈值，训练初期给予更大容忍度
    ...
    class InitState:
        ...
        # pos = [-3.0, 7.5, 2.0]  # 机器人初始出生的中心位置（平地过渡区,靠近吊桥侧）
