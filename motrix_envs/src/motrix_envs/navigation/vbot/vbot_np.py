...
class VbotEnv(NpEnv):
    ...
    def __init__(self, cfg: VBotEnvCfg, num_envs: int = 1):
        ...
        # 观测空间：67维（55 + 12维动作空间）
        # 观测空间：linvel(3) + gyro(3) + gravity(3) + joint_pos(12) + joint_vel(12) + 
    ...
    def _find_target_marker_dof_indices(self):
        ...
        # DOF 6-9: base quaternion (qx, qy, qz, qw) ← This is where the error occurs
        # DOF 10-21: joint angles
        # DOF 22-28: robot_heading_arrow freejoint (3 pos + 4 quat)
        # DOF 29-35: desired_heading_arrow freejoint (3 pos + 4 quat)
    ...
    def _find_arrow_dof_indices(self):
        ...
        arrow_init_height = self._cfg.init_state.pos[2] + 0.5  # 箭头初始高度
    ...
    def _init_contact_geometry(self):
        """初始化接触检测所需的几何体索引"""
        cfg = self._cfg
        print(f"Ground geom: {cfg.asset.ground_name}")
    ...
    def _init_termination_contact(self):
        ...
        #             print(f"Foot geom '{foot_name}' found at index {foot_idx}")
        #         else:
    ...
    def reset(self, data: mtx.SceneData, done: np.ndarray = None) -> tuple[np.ndarray, dict]:
        ...
        # print(f"obs.shape:{obs.shape}")  # Debug print removed
