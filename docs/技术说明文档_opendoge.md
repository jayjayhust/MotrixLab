...
- **训练总步数**: Section001 = 40 seconds / 4000 steps, Section01 = 120 seconds / 12000 steps
...
**路径点坐标配置（hard-4模式使用，分别涉及cfg_opendoge.py和xml文件的相关定义）:**

| 路径点名称 | 坐标位置 (X, Y, Z) | 作用 |
|-----------|-------------------|------|
| wp_1-1_body | (-3.0, 0.0, 0.1) | Section01第一排奖励点1 |
| wp_1-2_body | (0.0, 0.0, 0.1) | Section01第一排奖励点2 |
| wp_1-3_body | (3.0, 0.0, 0.1) | Section01第一排奖励点3 |
| wp_1-5_body | (-3.0, 4.5, 0.6) | Section01第二排奖励点1 |
| wp_1-6_body | (0.0, 4.5, 0.6) | Section01第二排奖励点2 |
| wp_1-7_body | (3.0, 4.5, 0.6) | Section01第二排奖励点3 |
| wp_1-4_body | (0.0, 8.0, 1.3) | Section01区域终点（驻留点） |
| wp_2-8_body | (0.3, 16.2, 1.8) | Section02奖励点1 |
| wp_2-12_body | (-0.2, 18.0, 1.3) | Section02过渡点 |
| wp_2-10_body | (-2.0, 17.8, 1.3) | Section02奖励点2 |
| wp_2-5-1_body | (-0.3, 20.2, 1.8) | Section02过渡点 |
| wp_2-1_body | (0.0, 24.0, 1.3) | Section02区域终点（驻留点） |
| wp_3-1_body | (0.0, 32.5, 1.5) | Section03区域终点（驻留点） |
...
12. wp_2-1_body (index=11, action=True) - 驻留点
13. wp_3-1_body (index=12, action=True) - 终点

**驻留点庆祝动作:**
- 持续时间: 60步 (约1秒@60Hz)
- 动作: 预定义的关节位置序列（机器狗四条腿同步小幅弯曲，作出下蹲动作）
- 不影响RL控制，仅用于可视化

**卡困检测机制 (Stuck Detection):**
- 检测窗口: 480步 (8秒@60Hz) - 记录机器人历史位姿信息
- 位置变化阈值: 0.15米 - 480步内移动小于0.15米视为卡困
- 旋转变化阈值: 15度 - 480步内旋转小于15度视为卡困
- 终止阈值: 连续240步检测到卡困后强制终止环境
- 实现方式: 环形缓冲区存储历史位置和旋转信息，实时比较当前位姿与历史位姿差异
- 作用: 防止机器人在复杂地形中陷入局部最优或卡住不动，提升训练效率和探索能力
...
| 文件名 | 说明 |
|-------|------|
| vbot_section001_np.py | 完善第一赛段环境实现（平地地形） |
| vbot_section002_waypoint_np.py | 完善第二赛段环境实现（全地形+途径点+庆祝动作） |
| cfg_opendoge.py | 添加第一赛段和第二赛段环境配置文件（地形、奖励、初始状态等） |
| cfgs_opendoge.py | 添加第一赛段和第二赛段PPO训练配置文件（超参数、网络结构等） |

#### 2.1.2 XML配置文件修改说明

**vbot.xml 修改内容（相对于vbot(original).xml）:**

| 修改项 | 原始配置 | 修改后配置 | 说明 |
|-------|---------|-----------|------|
| 传感器配置 | 基础传感器 | 新增5个传感器 | 增强感知能力 |
| 足端位置传感器 | 无 | 新增4个framepos传感器 | FR/FL/RR/RL_foot_pos，用于边缘距离计算 |
| 重力投影传感器 | 无 | 新增framezaxis传感器 | gravity_projection，用于坡度检测 |
| 可视化箭头 | 无 | 新增robot_heading_arrow和desired_heading_arrow | 绿色箭头显示当前朝向，蓝色箭头显示期望朝向 |

**scene_section002_waypoint.xml 修改内容（相对于scene_section01(original).xml）:**

| 修改项 | 原始配置 | 修改后配置 | 说明 |
|-------|---------|-----------|------|
| 路径点定义 | 无 | 新增20个waypoint body | wp_1-1到wp_3-1，用于路径点导航和奖励触发 |
| 接触传感器 | 基础15个 | 新增35个传感器 | 包含路径点接触检测和增强版足端接触传感器 |
| 路径点接触传感器 | 无 | 新增20个contact传感器 | wp_X-X_body_contact，用于检测机器人到达路径点 |
| 足端力传感器 | 无 | 新增8个force/normal传感器 | 支持更详细的接触力分析 |
